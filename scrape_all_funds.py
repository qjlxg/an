...
# 输出目录：结构改为 YYYYMM/YYYYMMDD
output_base_dir = os.path.join(year_month_dir, date_dir_name)
os.makedirs(output_base_dir, exist_ok=True) # 递归创建 YYYYMM 和 YYYYMMDD 目录
...
# 注意：这里假设 fund_data_dir 存在且包含有效的 csv 文件
...
    # 适配“大于等于7天”或“七天以上”等可能的表述
    redemption_fee_7d_str = fund.get('赎回费率_大于等于7天') or fund.get('赎回费率_7天以上') or fund.get('赎回费率_>=7天')
    redemption_fee_7d = clean_fee_rate(redemption_fee_7d_str)
    
    is_low_fee = True
    
    # 检查管理费
    if mgmt_fee is None:
        is_low_fee = False # 缺少数据视为不通过
    elif mgmt_fee > MAX_MANAGEMENT_FEE:
        is_low_fee = False
    
    # 检查托管费
    if cust_fee is None:
        is_low_fee = False
    elif cust_fee > MAX_CUSTODIAN_FEE:
        is_low_fee = False
        
    # 检查7天赎回费率
    if redemption_fee_7d is None:
        is_low_fee = False
    elif redemption_fee_7d > MAX_REDEMPTION_FEE_7D:
        is_low_fee = False
    
    if is_low_fee:
...
# 注意：这里使用 year_month_dir 作为 git add 的目标，以确保 YYYYMM 目录被跟踪
subprocess.run(['git', 'add', year_month_dir]) 
commit_result = subprocess.run(['git', 'commit', '-m', f"Add combined data and low-fee report for {date_dir_name} in {year_month_dir}"], capture_output=True, text=True)
