...
print(f"[{datetime.datetime.now(shanghai_tz).strftime('%H:%M:%S')}] 开始处理和写入主数据文件...")
...
try:
    ...
    print(f"[{datetime.datetime.now(shanghai_tz).strftime('%H:%M:%S')}] 成功保存合并文件: {output_path}")
...
print(f"[{datetime.datetime.now(shanghai_tz).strftime('%H:%M:%S')}] 开始筛选低费率基金并生成报告...")
...
if low_fee_funds:
    ...
    try:
        ...
        print(f"[{datetime.datetime.now(shanghai_tz).strftime('%H:%M:%S')}] 找到 {len(low_fee_funds)} 个低费率基金，已保存到报告文件: {report_path}")
    except Exception as e:
        print(f"写入低费率报告文件时发生错误: {e}")
else:
